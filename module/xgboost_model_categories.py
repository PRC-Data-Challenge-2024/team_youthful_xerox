...
def get_data(use_traffic):
    ...
    X_train = train.drop(
        columns=[
            "tow",
            "flight_id",
            "mean_tow_per_aircraft_type",
            "mean_tow_difference",
        ]
    )
    y_train = train["mean_tow_difference"]
    y_mean_train = train[
        ["aircraft_type", "tow", "mean_tow_per_aircraft_type", "mean_tow_difference"]
    ]
...
def xgboost_subgroup_models(use_traffic=True):
    ...
    for subgroup, aircraft_types in aircraft_subgroups.items():
        ...
        if subgroup_mask.sum() > 0:
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                X_subgroup, y_subgroup, test_size=0.3, random_state=42
            )
            ...
            y_pred_tow = (
                y_mean_train_subgroup["mean_tow_per_aircraft_type"]
                - mean_gt_tow_diff_pred
            )
    ...
    for subgroup, model in results.items():
        ...
        if submission_mask.sum() > 0:
            ...
            X_sub_group["tow_diff_pred"] = model.predict(
                X_sub_group.drop(
                    columns=[
                        "flight_id",
                        "mean_tow_per_aircraft_type",
            ...
            test_df = X_sub_group.merge(
                df_mean_per_aircraft,
                on="aircraft_type",
                how="left",
                suffixes=("", "_mean"),
            )
