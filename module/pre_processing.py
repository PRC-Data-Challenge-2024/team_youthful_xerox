...
airports_db = (
    airport_dataset_df[["icao", "elevation"]]
    .drop_duplicates(subset="icao")
    .rename(columns={"icao": "adep", "elevation": "dep_airport_altitude"})
)
...
mean_tow_per_type = (
    train_df_with_altitude.groupby("aircraft_type")["tow"].mean().reset_index()
)
train_df_merged_mean = train_df_with_altitude.merge(
    mean_tow_per_type, on="aircraft_type", suffixes=("", "_mean")
)

# Rename the new column
train_df_merged_mean.rename(
    columns={"tow_mean": "mean_tow_per_aircraft_type"}, inplace=True
)

# Compute the difference between mean tow and actual tow
train_df_merged_mean["mean_tow_difference"] = (
    train_df_merged_mean["mean_tow_per_aircraft_type"] - train_df_merged_mean["tow"]
)
...
median_tow_per_type = (
    train_df_with_altitude.groupby("aircraft_type")["tow"].median().reset_index()
)
train_df_merged_median = train_df_with_altitude.merge(
    median_tow_per_type, on="aircraft_type", suffixes=("", "_median")
)

# Rename the new column for median tow
train_df_merged_median.rename(
    columns={"tow_median": "median_tow_per_aircraft_type"}, inplace=True
)

# Compute the difference between median tow and actual tow
train_df_merged_median["median_tow_difference"] = (
    train_df_merged_median["median_tow_per_aircraft_type"]
    - train_df_merged_median["tow"]
)
