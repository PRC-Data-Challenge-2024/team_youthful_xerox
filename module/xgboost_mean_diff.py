...
def get_data(use_traffic):
    ...
    if use_traffic:
        features = pd.read_parquet("./features/trafic_features_v8/all_traffic_features_climb.parquet")#.dropna()
    ...
    X_train = train.drop(columns=["tow", "flight_id","mean_tow_per_aircraft_type","tow_difference"])
    y_train = train["tow_difference"]
    y_mean_train = train[["aircraft_type","tow","mean_tow_per_aircraft_type","tow_difference"]]
...
def xgboost_experimentation(use_traffic=True):
    ...
    X_submission["tow_diff"] = best_model.predict(X_submission.drop(columns=["flight_id","mean_tow_per_aircraft_type","tow_difference"])) 
    df_mean_per_aircraft = retreive_means()
    test_df = X_submission.merge(df_mean_per_aircraft, on='aircraft_type', how='left', suffixes=('', '_mean'))
