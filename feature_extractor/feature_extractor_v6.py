...
logger.add("processing_log.log", rotation="10 MB")  # Optional: logs to a file with rotation
...
def deduce_TAS(GS, WS, track, U_wind, V_wind):
    ...
    wind_angle = np.deg2rad(track) - wind_direction  # Relative angle between wind and track
...
def limit_data_duration(df: pl.DataFrame, time_column: str = 'timestamp', duration_minutes: int = 30) -> pl.DataFrame:
    """
...
def segment_signal_with_duration_check(dataframe, vertical_rate_threshold, discontinuity_threshold_seconds=2, max_duration_minutes=30):
    ...
    takeoff_phase_df = dataframe.filter(pl.col("vertical_rate") > vertical_rate_threshold)
    ...
    takeoff_phase_segments_lab = takeoff_phase_df.with_columns(
        (pl.col("timestamp").diff().fill_null(pl.duration(seconds=0))).alias("timestamp_diff")
    )
    ...
    takeoff_segments_with_duration = takeoff_phase_segments_lab.group_by("segment_id").agg([
        pl.col("timestamp").max().alias("end_time"),
        pl.col("timestamp").min().alias("start_time"),
        pl.len().alias("num_points"),
    ]).with_columns(
        ((pl.col("end_time").cast(pl.Int64) - pl.col("start_time").cast(pl.Int64)) / 1_000_000_000).alias("segment_duration_seconds")
    )
...
def process_flight(flight_df, flight_id, vertical_rate_threshold, min_duration_threshold_minutes, noise_tolerance, plot_output_dir=None, enable_plotting=True):

    # Sort the flight's data by timestamp
    flight_df = flight_df.sort("timestamp")
    columns_to_clip = ["altitude","groundspeed"]
    flight_df = flight_df.with_columns([pl.when(pl.col(col) < 0).then(0).otherwise(pl.col(col)).alias(col) for col in columns_to_clip])

    # Extract the actual takeoff phase based on vertical rate
    # segment_signal_with_duration_check(dataframe, vertical_rate_threshold, discontinuity_threshold_seconds=5, max_duration_minutes=30) 
    takeoff_segment  = segment_signal_with_duration_check(flight_df, vertical_rate_threshold, discontinuity_threshold_seconds=5, max_duration_minutes=30)
    ...
    time_diff = (takeoff_segment["timestamp"].max() - takeoff_segment["timestamp"].min()).total_seconds() / 60

    # Skip if the duration is less than the threshold in minutes
    if time_diff < min_duration_threshold_minutes:
        logger.warning(f"Skipping flight {flight_id} due to insufficient takeoff duration ({time_diff:.2f} minutes).")
        return None
    takeoff_segment = takeoff_segment.with_columns(
        (pl.col('u_component_of_wind')**2 + pl.col('v_component_of_wind')**2).sqrt().alias('wind_speed')
        )
    ...
    takeoff_segment = takeoff_segment.with_columns([
        ...
        pl.Series(groundspeed_diff).alias("groundspeed_diff"),  # Groundspeed difference
        pl.Series(track_deviation).alias("track_deviation"),  # Rate of change of track
        pl.Series([track_variance] * len(takeoff_segment)).alias("track_variance"),  # Track stability
    ])

    return takeoff_segment

# Function to lazily load and process each Parquet file
def process_parquet_file_lazy(file, vertical_rate_threshold=500, min_duration_threshold_minutes=1, noise_tolerance=50, plot_output_dir=None, enable_plotting=True):
    try:
        ...
        lazy_df = lazy_df.select([
            ...
            "temperature", "u_component_of_wind", "v_component_of_wind", "track"
        ])
        ...
        for flight_id in processed_df["flight_id"].unique():
            ...
            takeoff_segment = process_flight(flight_df, flight_id, vertical_rate_threshold, min_duration_threshold_minutes, noise_tolerance, plot_output_dir, enable_plotting)
...
def extract_takeoff_features_lazy(directory_path, output_path, noise_tolerance=50, vertical_rate_threshold=500, min_duration_threshold_minutes=1, plot_output_dir=None, enable_plotting=True):
    ...
    for file in tqdm(all_files, desc="Processing Parquet files"):
        takeoff_segment = process_parquet_file_lazy(file, vertical_rate_threshold, min_duration_threshold_minutes, noise_tolerance, plot_output_dir, enable_plotting)
        if takeoff_segment is not None:
            ...
            features = takeoff_segment.group_by("flight_id").agg([
                ...
                pl.col("track_variance").mean().alias("track_variance"),  # Track variance (constant)
                (pl.col("timestamp").max() - pl.col("timestamp").min()).alias("takeoff_duration")
            ])
            # Save the processed features for each file in the same structure and format as input
            output_file = os.path.join(output_path, os.path.basename(file))  # Keep the same filename
            features.write_parquet(output_file)
            logger.info(f"Processed and saved features for {file}")

# Example usage:
directory_path = "data"
output_path = "features/trafic_features_v6"  # Output directory where results will be saved
plot_output_dir = "reporting/plot_signals"  # Directory to save plots for skipped flights
...
extract_takeoff_features_lazy(directory_path, output_path,noise_tolerance, vertical_rate_threshold,min_duration_threshold_minutes, plot_output_dir)